...
class Game(wall: List[(Int, Int)], coin: List[(Int, Int, Int)], initialX: Int, initialY: Int) {
  ...
  def moveLeft(): Unit = {
    ...
    else if(board(positionX)(positionY) != 0){
      positionX-=1
   ...
   * A more advanced requirement would be to call checkCoins() if completed.
   */
  def moveLeft(n: Int): Unit = rpt(n)(moveLeft())
   ...
   * A more advanced requirement would be to call checkCoins() if completed.
   */
  def moveRight(n: Int): Unit = rpt(n)(moveRight())
   ...
   * A more advanced requirement would be to call checkCoins() if completed.
   */
  def moveUp(n: Int): Unit = rpt(n)(moveUp())
   ...
   * A more advanced requirement would be to call checkCoins() if completed.
   */
  def moveDown(n: Int): Unit = rpt(n)(moveDown())
  ...
  def checkCoin(): Unit = {
    if(board(positionX)(positionY) > 0){
      points = points + board(positionX)(positionY)
  ...
  def move(s: String): Unit = {
    for(letter <- s){
      if(letter == 'w') moveUp()
      else if(letter == 's') moveDown()
      else if(letter == 'a') moveLeft()
      else if(letter == 'd') moveRight()
      else save()
    }
  }
   ...
   */
  def checkCoins(): Unit = {
    if(Math.abs((positionX-saveX+1)*(positionY-saveY+1))>=9 && saveX >= 0 && saveY >= 0){
      if(positionX >= saveX && positionY >= saveY){
        ...
        for(x <- positionX to saveX; y <- saveY to positionY) {
          if(board(x)(y) > 0){
            points = points + board(x)(y)
            board(x)(y) = -1
          }
        ...
        for(x <- saveX to positionX; y <- positionY to saveY) {
          if(board(x)(y) > 0){
            points = points + board(x)(y)
            board(x)(y) = -1
          }
      ...
      else for(x <- positionX to saveX; y <- positionY to saveY) {
        if(board(x)(y) > 0){
          points = points + board(x)(y)
          board(x)(y) = -1
        }
  ...
  def suggestSolution(): String = {
    ...
    for (x <- 0 until 10) {
      for (y <- 0 until 10) {
        if (board(x)(y) > 0 && points < maxPoints()-1) {
          points += board(x)(y)
          path += suggestMove(x, y)
          positionX = x
          positionY = y
        }
      }
  ...
  def suggestMove(x: Int, y: Int): String = {
    ...
    if(x > -1 && x < 10 && y > -1 && y < 10){
      ...
      if(positionX >= x && positionY >= y){
        ...
        positionX = saveX
        positionY = saveY
        if(suggestion.isEmpty) points = 0
        return suggestion
      }
...
object GameBuilder {
  ...
  def initialiseGame2(): Game = {
    return new Game(List((3, 3), (3, 4), (3, 5), (5, 3), (5, 4), (5, 4)), List((4, 4, 20), (6, 3, 20)), 3, 2)
  }
